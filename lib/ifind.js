...
// 密钥解析链: 环境变量 IFIND_MCP_KEY → 项目根 ifind_key.json → ifindSkillPath(显式值, 'auto'跳过) → homedir已知技能位置自动发现
function resolveAuth() {
  ...
  const candidates = [];
  if (CFG.ifindSkillPath && CFG.ifindSkillPath !== 'auto') {
    candidates.push(path.join(path.dirname(CFG.ifindSkillPath), 'mcp_config.json'));
  }
  for (const dir of ['.zcode', '.claude', '.agents']) {
    candidates.push(path.join(os.homedir(), dir, 'skills', 'ifind-finance-data', 'mcp_config.json'));
  }
  for (const p of candidates) {
    try { return JSON.parse(fs.readFileSync(p, 'utf-8')).auth_token; } catch {}
  }
  throw new Error('未找到 iFinD 密钥: 设置环境变量 IFIND_MCP_KEY, 或在项目根创建 ifind_key.json {"auth_token":"..."} , 或在 ~/.zcode|~/.claude|~/.agents 安装 iFind 技能');
}
...
function colKey(h) {
  ...
  if (s.includes('主力') || s.includes('资金流向') || s.includes('净流入')) return 'flow';
  if (s.includes('市值')) return 'mcap';
  if (s.includes('换手')) return 'turnover';
