...
function scoreStock(ds, ctx = {}) {
  ...
  const boardPenalty = ds.boardQ?.broken ? 8 : 0; // 触及涨停未封住(炸板): 上方抛压与分歧的直接证据
  const composite = clamp(rsScore * 0.4 + posScore * 0.3 + healthScore * 0.3 - blowPenalty - boardPenalty);
  return {
    composite: Math.round(composite),
    grade: composite >= 75 ? '强（可操作）' : composite >= 55 ? '中（观察）' : '弱（放弃）',
    parts: { rsScore: Math.round(rsScore), rsExcess: { vsIndex: r2(exIdx), vsBoard: r2(exBoard) }, posScore, healthScore: Math.round(clamp(healthScore)), blowPenalty, boardPenalty },
    summary: `${ds.posTag} | 20日区间${ds.posPct.toFixed(0)}%分位 | 涨量/跌量比 ${isFinite(udr) ? udr.toFixed(2) : '-'}${ds.boardQ?.sealed ? ' | 收盘封板' : ''}${ds.boardQ?.broken ? ' | ⚠炸板回落' : ''}${ds.flow && isFinite(ds.flow.today) ? ` | 主力今日${ds.flow.today >= 0 ? '+' : ''}${(ds.flow.today / 1e8).toFixed(2)}亿` : ''}${ds.blowoff ? ` | ⚠天量长阴遗迹(${ds.blowoff.date},${ds.blowoff.daysAgo}日前)` : ''}`,
  };
}
...
function buildCards(code, name, ds, temp, stage) {
  ...
  // 锚点分级: 昨日涨停票的MA10锚常在-8%~-15%, 对T+1买入者是接刀——改浅锚(-3%~-5%), 风控线-6%与急跌否决同线
  const wasLimitUp = ds.pctToday >= limitUpPct(code) - 0.15 || (ds.boardQ && ds.boardQ.sealed);
  let bZoneLo, bZoneHi, bStop, anchorNote;
  if (wasLimitUp) {
    const sa = P.shallowAnchor || { lo: -5, hi: -3 };
    bZoneLo = r2(ds.close * (1 + sa.lo / 100)); bZoneHi = r2(ds.close * (1 + sa.hi / 100));
    bStop = r2(ds.close * (1 + (P.shallowStopPct || -6) / 100));
    anchorNote = `昨日涨停(${ds.pctToday.toFixed(1)}%)→浅锚-3%~-5%, 风控线-6%与急跌否决同线`;
  } else {
    bZoneLo = r2(key * 0.99); bZoneHi = r2(key * 1.01);
    // 止损可执行性下限: 不得低于跌停价+0.02(否则跌停封死时止损形同虚设)
    const ldPct = /^(30|68)/.test(String(code)) ? 0.8 : 0.9;
    const limitDown = Math.round(ds.close * ldPct * 100) / 100;
    bStop = Math.max(r2(key * (1 - P.stopBelowKeyPct)), r2(limitDown + 0.02));
    anchorNote = '深锚MA10/平台';
  }
  ...
  return {
    ...
    B: {
      ...
      confirm: '企稳v2(低点抬升+收复15分钟VWAP)+量价结构(回升放量)+大盘/板块≥-1%+主力未大幅流出',
      anchor: anchorNote,
      desc: '回踩关键位低吸',
    },
    guardNotes: guards,
