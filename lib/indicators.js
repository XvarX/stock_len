...
function evaluateConfirmations(minBars, opt = {}) {
  ...
  // B卡量价结构: 近6分钟 回升分钟均量 vs 下跌分钟均量——低吸要看见承接(回升带量), 不是"没有抛压"就行
  const upV = [], dnV = [];
  for (let i = minBars.length - 6; i < minBars.length; i++) {
    if (i <= 0) continue;
    if (minBars[i].close > minBars[i - 1].close) upV.push(minBars[i].volume);
    else if (minBars[i].close < minBars[i - 1].close) dnV.push(minBars[i].volume);
  }
  res.bounceVolOk = dnV.length === 0 ? null : (upV.length > 0 && upV.reduce((s, x) => s + x, 0) / upV.length >= dnV.reduce((s, x) => s + x, 0) / dnV.length);
  return res;
}
