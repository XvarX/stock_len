...
async function scanLeaders(f) {
  ...
  const candidates = probe
    ...
    .map((m) => m.code);
  return { ctx, halted: false, boards: chosen.map((b) => ({ name: b.name, hits: b.hits, codes: [...(counts[b.name] || [])] })), conceptsOf, rawGainCount: movers.length, marketLimitUpCount, probed: probe.length, candidates };
}
...
function buildReview(plan, analyzed, alerts) {
  ...
  for (const it of plan.items) {
    ...
    const infoBlock = evs.some((e) => e.kind.startsWith('INFO·'));
