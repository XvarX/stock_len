...
/** --scan 自动找热度板块与候选龙头(资金口径: 涨停票涨幅并列, 主力净流入才是有效排序键) */
async function scanLeaders(f) {
  const ctx = await marketContext(f);
  if (ctx.temp.overall === 'defend') return { ctx, halted: true, reason: '温度计防守档——今天整体休息，不产生候选池' };
  // ① 大涨+资金+流动性一次筛选(选股语句内置主力净流入>0/市值/换手条件, 实测单次可回100行)
  const q = `今日涨幅大于${CFG.scan.bigGainPct}%且主力净流入额大于0且总市值大于${CFG.scan.minMarketCapYi}亿且换手率大于${CFG.scan.minTurnoverRate}%的非ST股票`;
  ...
  const flowSort = (a, b) => (isFinite(b.flow) ? b.flow : -Infinity) - (isFinite(a.flow) ? a.flow : -Infinity);
  const movers = rows
    .map((x) => ({ code: String(x.code || '').split('.')[0], name: x.name || '', pct: num(x.pct), flow: num(x.flow) }))
    .filter((x) => /^\d{6}$/.test(x.code) && isFinite(x.pct))
    .sort((a, b) => flowSort(a, b) || b.pct - a.pct);
  if (!movers.length) return { ctx, halted: true, reason: `今日涨幅>${CFG.scan.bigGainPct}%且主力净流入>0的股票为空或查询失败——无短线热度` };
  // ② 概念聚合(资金前probeN作聚类样本, 解决涨停并列下小样本随机性)
  ...
  // ③ 候选=热点板内大涨股按资金序(次序用涨停制度归一化涨幅, 消除10/20/30cm板块偏置)
  const limitPct = (code) => /^(68|30)/.test(code) ? 20 : /^(92|43|83|87)/.test(code) ? 30 : 10;
  const candidates = probe
    .filter((m) => (conceptsOf[m.code] || []).some((cpt) => chosen.some((b) => b.name === cpt)))
    .sort((a, b) => flowSort(a, b) || b.pct / limitPct(b.code) - a.pct / limitPct(a.code))
    .slice(0, CFG.scan.candidateN)
    .map((m) => m.code);
  return { ctx, halted: false, boards: chosen, rawGainCount: movers.length, probed: probe.length, candidates };
}
