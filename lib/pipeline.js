...
async function scanLeaders(f) {
  ...
  const candidates = probe
    ...
    .map((m) => m.code);
  return { ctx, halted: false, boards: chosen.map((b) => ({ name: b.name, hits: b.hits, codes: [...(counts[b.name] || [])] })), conceptsOf, rawGainCount: movers.length, probed: probe.length, candidates };
}
