...
function renderFull(ctxObj, title) {
  ...
  if (result.scanMeta) {
    ...
    L.push(`## 龙头识别v2 · 情绪与梯队`);
    L.push(`- 情绪定位: **${sm.emotion.label}** | 涨停池口径 ${sm.limitUpCount} 只 | 最高连板 ${sm.ladder.max} 板 | 梯队: 首板${sm.ladder.first} / 2板${sm.ladder.second} / 3板+${sm.ladder.thirdPlus}`);
    if (sm.purity && sm.purity.length) {
      L.push(`- 主线纯度: ` + sm.purity.map((x) => `${x.name}=${x.breadth ?? '?'}只成分股(${x.tier})`).join(' | '));
      L.push(`- ⚠ 榜单候选按概念标签聚类, 宽概念(数百成分股)下蹭标签票难免——正宗性以会话人工判定为准(主业×催化事件对照)`);
    }
    if (sm.catalystBoard) {
      L.push(sm.catalyst && sm.catalyst.count
        ? `- 催化确认[${sm.catalystBoard}]: 近3日相关资讯 ${sm.catalyst.count} 条 | 首条: ${sm.catalyst.headline}`
        : `- 催化确认[${sm.catalystBoard}]: 近3日无新闻共振(纯技术脉冲, 已降权)`);
    }
    L.push(`\n### 龙头榜(时序25/高度30/带动20/质量15/资金10)`);
    for (const ld of sm.leaders) {
      L.push(`- #${ld.rank} ${ld.code} ${ld.name} — 龙头分 **${ld.score}**(${ld.grade}) | ${ld.boards}连板 首板${ld.firstDate} | 带动${ld.clusterHits}只${ld.oneWordToday ? ' | 一字板' : ''}${ld.broken ? ' | ⚠炸板' : ''}`);
    }
