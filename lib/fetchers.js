...
class Fetchers {
  ...
  _cached(key, fn) {
    ...
    try { return Promise.resolve(JSON.parse(fs.readFileSync(file, 'utf-8'))); }
    catch {
      return fn().then((v) => { try { fs.mkdirSync(this.cacheDir, { recursive: true }); fs.writeFileSync(file, JSON.stringify(v)); } catch {} return v; });
    }
  }
  ...
  async board(name) {
    ...
    for (const r of today.rows || []) for (const [k, v] of Object.entries(r)) if (k.includes('涨跌幅')) pctToday = num(v);
    ...
    if (ser.rows && ser.rows.some((x) => Object.values(x).some((v) => /^\d{8}$/.test(String(v))))) {
      for (const x of ser.rows) {
        ...
        const pk = Object.keys(x).find((k) => k.includes('涨跌幅'));
