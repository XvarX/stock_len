...
class Fetchers {
  ...
  /** 多主体日线(≤4只/包, 配额友好): 返回 {code: bars升序}——回测专用, 窗口短防截断 */
  async dailyBatch(codes, calDays = 32) {
    const out = {};
    const p = (x) => String(x).padStart(2, '0');
    const end = new Date(), start = new Date(Date.now() - calDays * 86400000);
    const fmtCn = (d) => `${d.getFullYear()}年${p(d.getMonth() + 1)}月${p(d.getDate())}日`;
    for (let i = 0; i < codes.length; i += 4) {
      const chunk = codes.slice(i, i + 4);
      const q = `${chunk.map((c) => c).join('、')}从${fmtCn(start)}到${fmtCn(end)}的每日开盘价、最高价、最低价、收盘价、涨跌幅、成交量`;
      let r;
      try { r = await this._tableOf('stock', 'get_stock_performance', { query: q }, `B${chunk.join('|')}`); }
      catch { continue; }
      if (!r.ok || !r.rows) continue;
      for (const x of r.rows) {
        const code = String(x.code || '').split('.')[0];
        if (!/^\d{6}$/.test(code) || !isFinite(num(x.close))) continue;
        (out[code] = out[code] || []).push({ date: x.date, open: num(x.open), high: num(x.high), low: num(x.low), close: num(x.close), pct: num(x.pct), volume: num(x.volume) });
      }
    }
